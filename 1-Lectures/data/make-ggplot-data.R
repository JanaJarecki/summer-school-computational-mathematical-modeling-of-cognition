...
d[, delta := runif(.N, 0, 5)]
d[, model_X_pred := runif(.N,.1,.5) * 0^(choice) + runif(.N,.3,.8) * 0^(1-choice)]
d[, model_Y_pred := runif(.N,.4,.8) * 0^(choice) + runif(.N,.1,.7) * 0^(1-choice)]
d[, mean(model_X_pred), by=choice]


d[, condition := factor(condition, labels = c("A","B"))]

fwrite(d, "data/data_ggplot.csv")
